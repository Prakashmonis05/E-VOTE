...
export default function AdminSignupPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await fetchApi('/auth/admin/signup', {
        method: 'POST',
    ...
    <div className="max-w-md mx-auto my-12">
      <div className="glass-panel p-8 rounded-3xl border border-emerald-500/20 space-y-6 shadow-2xl">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-slate-400">Register new administrator email profile</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full bg-slate-900/80 border border-slate-800 focus:border-emerald-500 rounded-xl px-4 py-3 text-white placeholder-slate-600 focus:outline-none transition-colors"
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-slate-300 uppercase tracking-wider mb-2">
